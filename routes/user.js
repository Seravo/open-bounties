  ...
  validateName(req.body.name, function(err, name) {
    ...
    validateMail(req.body.email, function(err, email) {
      ...
      validateUsername(req.body.username, function(err, username) {
        ...
        if (typeof(name) === 'undefined' || typeof(username) === 'undefined' || typeof(email) === 'undefined') {
...
function validateMail(mail, callback) {
  if (mail.length === 0) {
    ...
    return callback(err);
  } else return callback(null, mail)
}
...
exports.user = function(req, res, next, id) {
  ...
  models.User.findOne({
    ...
    // word 'user' is reserved, cannot be used here, it leads to some authentication problems
    // req.user = user;  
    req.myUser = user;
    next()
